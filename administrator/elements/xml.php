    ...
    protected function getInput() {
        ...
        if ($execute == 'manifestupdate') {
			...
			if (!version_compare($joomlaVersion, "4.0.0", ">=")) {
				...
				$xmlFile = file_get_contents($basePath.'mod_digi_contact.xml');
				...
				if ($xmlFile && $changed) {
					JFile::write($basePath . 'mod_digi_contact.xml' , $xmlFile);
				...
				$message .= JText::_('MOD_DIGI_CONTACT_FIELD_UPDATE_XML_DONE_DESC');
				$message .= '</p>';
				return $message;
			...
			if (!version_compare($joomlaVersion, "4.0.0", ">=")) {
				...
				if (strpos($xmlFile, 'layout="joomla.form.field.radio.switcher"') !== false) {
					...
					$update_button .= JText::_('MOD_DIGI_CONTACT_FIELD_UPDATE_XML_DESC').'<br /><br />';
					$update_button .= '<a class="btn btn-warning btn-large" href="'.$uri.'&execute=manifestupdate">'.JText::_('MOD_DIGI_CONTACT_FIELD_UPDATE_XML_TITLE').'</a>';
					$update_button .= '</p>';
