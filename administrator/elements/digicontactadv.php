...
class JFormFieldDigicontactadv extends JFormField {
    protected $type = 'Digicontactadv';
    ...
    protected function getInput() {
		...
		$document->addStyleDeclaration('
			#digigreg_adv {
				...
				background-image: url("'.URI::root().'modules'.DS.'mod_digi_contact'.DS.'assets'.DS.'images'.DS.'pattern-suggestion.png");
			...
			#digigreg_donation {
				...
				background-image: url("'.URI::root().'modules'.DS.'mod_digi_contact'.DS.'assets'.DS.'images'.DS.'pattern-donation.png");
				background-repeat: repeat;
				position: relative;
				z-index: 0;
				display: flex;
        ...
        $html .= '<h2 class="'.$card_title_class.' text-light">'.strToUpper(JText::_('MOD_DIGI_CONTACT_DONATION_TITLE')).'</h2>';
        
        $html .= '<p class="'.$card_text_class.' bg-light text-dark rounded p-3">'.JText::_('MOD_DIGI_CONTACT_DONATION_DESC').'<br /><small><i class="text-dark">'.JText::_('MOD_DIGI_CONTACT_DONATION_SIGN').'</i></small>'.'</p>';
        ...
        $html .= '<a class="btn btn-outline-light" target="_blank" href="'.$paypal_link.'"><img class="btn-img" src="'.URI::root().'modules'.DS.'mod_digi_contact'.DS.'assets'.DS.'images'.DS.'logo-paypal.png" alt="PayPal" />PayPal</a>';
        $html .= '<a class="btn btn-outline-light" target="_blank" href="'.$revolut_link.'"><img class="btn-img" src="'.URI::root().'modules'.DS.'mod_digi_contact'.DS.'assets'.DS.'images'.DS.'logo-revolut.png" alt="Revolut" />Revolut</a>';
        $html .= '</span>';
        $html .= '</p>';
        ...
        $html .= '<h2 class="'.$card_title_class.' text-light">'.strToUpper(JText::_('MOD_DIGI_CONTACT_ADV_TITLE')).'</h2>';
        
        $html .= '<p class="'.$card_text_class.' bg-light text-dark rounded p-3">'.JText::_('MOD_DIGI_CONTACT_ADV_DESC').'</p>';
        
		$html .= '<p class="'.$card_text_class.' bg-warning text-dark rounded p-3">';
        $html .= '<span class="text-uppercase">'.JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_ASK_QUESTION_BEFORE_TO_BUY').'</span> <br />';
        $html .= '<a class="btn '.$btn_dark_class.' btn-sm mt-1" href="'.$support_question_link.'" target="_blank">'.strToUpper(JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_ASK_QUESTION')).'</a> <br /><br />';
        $html .= '<span class="text-uppercase">'.JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_BUY_HOURS').'</span> <br />';
        $html .= '<span class="btn-group btn-group-sm mt-1" role="group">';
        $html .= '<a class="btn '.$btn_dark_class.'" target="_blank" href="'.$support_one_hour_link .'">'.strToUpper(JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_ONE_HOUR')).'</a>';
        $html .= '<a class="btn '.$btn_dark_class.'" target="_blank" href="'.$support_two_hours_link .'">'.strToUpper(JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_TWO_HOURS')).'</a>';
        $html .= '<a class="btn '.$btn_dark_class.'" target="_blank" href="'.$support_three_hours_link .'">'.strToUpper(JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_THREE_HOURS')).'</a>';
        $html .= '<a class="btn '.$btn_dark_class.'" target="_blank" href="'.$support_five_hours_link .'">'.strToUpper(JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_FIVE_HOURS')).'</a>';
        $html .= '<a class="btn '.$btn_dark_class.'" target="_blank" href="'.$support_ten_hours_link .'">'.strToUpper(JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_TEN_HOURS')).'</a>';
        $html .= '</span> <br /><br />';
        $html .= '<span class="text-uppercase">'.JText::_('MOD_DIGI_CONTACT_ADV_SUPPORT_WORKING_HOURS').'</span>';
        $html .= '</p>';
